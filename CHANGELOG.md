...
- ✅ 🧪 **A.4** Панель управления (паритет легаси `Admin\DashboardController@index` + `DashboardTrait`): `entities/admin-dashboard` (`adminDashboardQueryOptions` → `GET /admin/dashboard`) + `pages/admin-dashboard` (`/admin`, гейт `requireRole('admin')`) — приветственный hero с быстрыми кнопками (Комментарии/Тикеты/Жалобы) + «Очистить кэш»; 3 метрик-карты (ежедневные продажи по типу курса / доход платформы / количество продаж), 4 счётчика (новая продажа/комментарий/обращение/курсы на проверке), график «Статистика продаж» (переключатель Месяц/Год) + «Последние комментарии», 3 карты (последние обращения/онлайн-курсы/курсы со статус-бейджами), график регистраций пользователей. Новый `shared/ui/LineChart` (инлайн-SVG, CSP-safe). Сайдбар: «Панель управления» в секции «Администрирование». Тесты: `AdminDashboardPage`
- ✅ 🧪 **A.5** Маркетинговая панель (паритет легаси `Admin\DashboardController@marketing`): `entities/admin-marketing` (`adminMarketingQueryOptions` → `GET /admin/marketing`) + `pages/admin-marketing` (`/admin/marketing`, гейт `requireRole('admin')`) — 4 счётчика (пользователи без покупок / инструкторы без курсов / рекомендуемые курсы / активные скидки), график «Статистика чистой прибыли (без налога)» (переключатель Месяц/Год) + прибыль за сегодня/неделю/месяц/год с grow%, панель «Типы курсов» (пончик + легенда %), таблицы топ-продаж (курсы / встречи / инструкторы / организации / самые активные студенты) с кросс-ссылками на модерацию/пользователей. Новый `shared/ui/PieChart` (инлайн-SVG donut, CSP-safe). Сайдбар: «Маркетинговая панель» в секции «Администрирование». Тесты: `AdminMarketingPage`
- ✅ 🧪 **A.6** Управление курсами (паритет легаси `Admin\WebinarController@index`): `entities/admin-course-manage` (`adminCoursesManageQueryOptions(filters)` → `GET /admin/courses/manage`, `adminLiveSessionsQueryOptions`) + `pages/admin-courses-manage` — общий экран для трёх модулей **Курсы** (`/admin/courses-list`, type=course), **Онлайн курсы** (`/admin/online-courses`, type=webinar), **Текстовые курсы** (`/admin/text-courses`, type=text_lesson): 4 карточки-статы (всего/на проверке/суммарная длительность HH:MM/всего продаж), панель фильтров (поиск/дата начала/дата окончания/сортировка/категория/статус + «Показать результаты»), таблица (ID/заголовок+категория/инструктор/цена/продажи+вместимость/доход/студенты/создано/обновлено/статус-бейдж/действия), экспорт в CSV, пагинация, меню действий (Редактировать/Одобрить/Отклонить/Снять с публикации/Удалить). Отдельная страница **История живых сессий** (`/admin/live-sessions`, гейт-стаб пустой). Сайдбар: аккордеон «Курсы» в секции «Администрирование». `features/moderate-courses` дополнен `useDeleteCourse` + `DELETE`. Тесты: `AdminCoursesManagePage`, `AdminLiveSessionsPage`
- ✅ 🧪 **A.7** Новый курс для админа (паритет `Admin\WebinarController@create`) — **одностраничная форма** как в легаси-админке: `pages/admin-course-form` (`/admin/course/new`, гейт `requireRole('admin')`) с секциями **Основная информация** (язык/тип/название/баллы/URL/инструктор/SEO/миниатюра·обложка·иконка/демо-видео/краткое·полное описание), **Дополнительная информация** (вместимость/дата·продолжительность/часовой пояс/тумблеры поддержка·сертификат·скачивание·совместный преподаватель·форум·подписка·приватное·только для студентов·список ожидания/период доступа/цена/категория), **Сообщение рецензенту** + зелёная «Сохранить и продолжить» → создаёт черновик (`draft`) и ведёт к наполнению (`/instructor/course/$id/edit?step=4`). Селектор инструктора из `courseTeachersQueryOptions`. Кнопка «Новый курс» в списке курсов ведёт сюда. Также в `Step1Basic` визарда есть админ-селектор «Инструктор»; `teacher_id` протянут в конфиг визарда. Тесты: `AdminCourseFormPage`, `config` (teacher_id). NOTE: «Отображаемое количество продаж» и «Теги» не перенесены (нет в модели)
- ✅ 🧪 **A.8** Пакеты курсов для админа (bundles, паритет `Admin\BundleController`): **Новый** — `pages/admin-bundle-form` (`/admin/bundle/new`) одностраничная форма как в легаси (Основная информация: язык/название/необходимые баллы/URL пакета/инструктор/SEO/миниатюра·обложка/демо-видео/краткое·полное описание; Дополнительная: тумблеры сертификат·приватное·только для студентов·подписка + период доступа/цена/категория; Сообщение рецензенту + зелёная «Сохранить и продолжить» → `POST /panel/bundles`, ведёт в список). **Список** — `pages/admin-bundles` (`/admin/bundles`) — таблица всех пакетов (ID/название/инструктор/категория/курсов/цена/статус + удаление) через `adminBundlesQueryOptions` → `GET /admin/bundles`. `features/manage-bundle` дополнен `useCreateBundle`/`adminBundlesQueryOptions`. Сайдбар: аккордеон «Пакеты курсов» (Новый/Список). Тесты: `AdminBundleFormPage`, `AdminBundlesPage`
...
- ✅ 🧪 **P.14** Тесты инструктора — редизайн 1:1 с легаси: **Новый тест** (`/instructor/quizzes/new`) — двухколоночная форма (`QuizForm`): слева «Основная информация» (Курс/Название/Описание) + «Настройки теста» (Время/Попытки/Проходной балл/Срок действия + тумблеры случайный порядок·сертификат·активный), справа панель «Вопросы теста»; **Список тестов** (`/instructor/quizzes`) — 3 карточки-метрики (Тесты/Вопросы/Студенты) + таблица (Название·курс/Вопросы/Время/Общий балл/Проходной/Студенты/Статус·бейдж/Дата/Действия) с меню **Редактировать/Удалить**; **редактирование** (`/instructor/quizzes/$quizId/edit`). Сайдбар «Тесты» → Новый тест / Список. Тесты: `InstructorQuizzesPage`
- ✅ 🧪 **P.13** Дашборд инструктора (паритет legacy instructor dashboard): `/panel` для роли инструктора рендерит свой вид (`InstructorDashboardView`) — hello-box с 4 счётчиками (Курсы/Встречи/Товары/Пакеты курсов), **«Управление курсами»** (карточки курсов с числом студентов + «Просмотреть детали» + «Создать курс»), **«Обзор курсов»** по типам, карточка кошелька и блок продаж. Данные из `GET /panel/instructor-dashboard` (`entities/dashboard.instructorDashboardQueryOptions`). Форма создания курса (`CourseForm` / `/instructor/course/new`) уже была — добавлены входы «Создать курс» (сайдбар «Преподавание» + CTA на дашборде). Тест: `InstructorDashboardView`
- ✅ 🧪 **P.12** «Стать инструктором» (паритет легаси Become Instructor): `pages/become-instructor` (`/become-instructor`, для студентов) — выбор роли (Инструктор/Организация) + специализации (категории) + о себе → `features/become-instructor` (`POST /panel/become-instructor`); статусы заявки (на рассмотрении / одобрена / отклонена) и «Вы уже инструктор». Админ-модерация: `pages/admin-become-instructors` (`/admin/become-instructors`, `requireRole('admin')`) — список заявок + Одобрить/Отклонить (accept меняет роль студента). `entities/become-instructor`. Сайдбар: «Стать инструктором» (секция «Карьера», только студенты) + «Заявки инструкторов» (админ). Тесты: `BecomeInstructorPage`
