...
- ✅ 🧪 **A.7** Новый курс для админа (паритет `Admin\WebinarController@create`): переиспользуем визард `manage-course` (`/instructor/course/new`, `/instructor/course/$id/edit` — админ проходит гейт после бэкенд-изменения). В `Step1Basic` добавлен админ-селектор **«Инструктор»** (виден только для `role_name==admin`, опции из `courseTeachersQueryOptions` → `GET /admin/courses/teachers`); `teacher_id` протянут в `WizardForm`/`initialForm`/`buildCreateBody`/`buildStepBody`. Кнопки «Новый курс»/«Редактировать» в списке курсов ведут в визард. Тест: `config` (teacher_id в create/step-1 body)
